...
class CLIP(nn.Module):
    def __init__(
        ...
        v_dim = int(vision_encoder.config.hidden_size)
        t_dim = int(text_encoder.config.hidden_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        t_h = t_out.last_hidden_state.to(dtype=img_feat.dtype)
        mask = attention_mask.unsqueeze(-1).to(dtype=t_h.dtype)
        txt_pooled = (t_h * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1)
        txt_feat = self.text_proj(txt_pooled)
...
def train(
    ...
    vision_encoder = vlm.model.model.vision_model
    text_encoder = vlm.model.model.text_model
    clip_dtype = torch.bfloat16 if device == "cuda" else torch.float32
    model = CLIP(vision_encoder, text_encoder).to(device=device, dtype=clip_dtype)
    ...
    # load dataset
    train_dataset = CaptionDataset("train", data_dir, max_samples=max_train_samples)
